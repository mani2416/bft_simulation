        ...
        loop{
            ...
            if let Some(event) = event{
                ...
                match event.event_type{
                    ...
                    EventType::Network => warn!(target: "simulation", "Network event still unimplemented"),
                    EventType::Reception(r) => {
                        self.update_time(event.time);
                        let receiver = self.node_map.get_mut(&r.id).expect("A message was sent to a non-existent node id");
