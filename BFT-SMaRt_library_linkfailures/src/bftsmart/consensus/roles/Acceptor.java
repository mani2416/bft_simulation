...
public final class Acceptor {
    ...
    private Logger eval_logger = LoggerFactory.getLogger("eval_pbft");
...
//        eval_logger.info("Incoming consensus message for acceptor: " + msg.toString());
...
//            eval_logger.debug("Out of context msg with id " + msg.getNumber());
...
//    	eval_logger.info("PROPOSE for consensus " + cid);
    ...
    private void executePropose(Epoch epoch, byte[] value) {
        ...
        if(epoch.propValue == null) { //only accept one propose per epoch
            ...
            // received a propose => R1
            eval_logger.info("R1;" + epoch.getConsensus().getId() + ";" + epoch.getTimestamp());
...
//        logger.debug("WRITE from " + a + " for consensus " + cid);
        eval_logger.debug("WRITE from " + a + " for consensus " + cid + ";x;x");
    ...
    private void computeWrite(int cid, Epoch epoch, byte[] value) {
        ...
        eval_logger.debug("I have " + writeAccepted +
                " WRITEs for " + cid + "," + epoch.getTimestamp() + " need " + controller.getQuorum()+1+";x;x");

        if (writeAccepted > controller.getQuorum() && Arrays.equals(value, epoch.propValueHash)) {
                        
            if (!epoch.isAcceptSent()) {

                eval_logger.info("Q2;" + cid + ";" + epoch.getTimestamp());
                
                logger.debug("Sending WRITE for " + cid); //WRONG! it's an accept
    ...
    private void acceptReceived(Epoch epoch, ConsensusMessage msg) {
        ...
        eval_logger.debug("ACCEPT from " + msg.getSender() + " for consensus " + cid+";x;x");
    ...
    private void computeAccept(int cid, Epoch epoch, byte[] value) {
        ...
        eval_logger.debug("I have " + epoch.countAccept(value) +
                " ACCEPTs for " + cid + "," + epoch.getTimestamp() + " need " + controller.getQuorum()+1 + ";x;x");

        if (epoch.countAccept(value) > controller.getQuorum() && !epoch.getConsensus().isDecided()) {
            eval_logger.info("Q3;" + cid + ";" + epoch.getTimestamp());
