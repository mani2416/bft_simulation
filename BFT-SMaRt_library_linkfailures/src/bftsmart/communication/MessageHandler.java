...
public class MessageHandler {
    ...
    private Logger eval_pbft = LoggerFactory.getLogger("eval_pbft");
    ...
    protected void processData(SystemMessage sm) {
        if (sm instanceof ConsensusMessage) {
            ...
            // Let's lose some (that are not sent to myself!)
            double p = this.random.nextDouble();
            if (p > 0.95 && sm.getSender() != myId){
                eval_pbft.debug("Omission of consensus message: " + ((ConsensusMessage) sm).toString());
                if ( ((ConsensusMessage) sm).getType() == MessageFactory.PROPOSE ){
                    try {
                        Thread.sleep(30000);
                    }catch (Exception e){
                        e.printStackTrace();
...
//            eval_pbft.info("Incoming consensus message: " + consMsg.toString());
